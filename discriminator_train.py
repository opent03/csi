...
def load_data(data_dir):
    ...
    y_train_d, y_train_loc, y_train_act = y_train[:, 0], y_train[:, 1], y_train[:, 2]
    y_test_d, y_test_loc, y_test_act = y_test[:, 0],y_test[:, 1], y_test[:, 2]
    ...
    y_train_d = np.reshape(y_train_d, (num_train_instances, 1))
    train_label = np.concatenate((y_train_act, y_train_loc, y_train_d), 1)
    ...
    y_test_d = np.reshape(y_test_d, (num_test_instances, 1))
    test_label = np.concatenate((y_test_act, y_test_loc, y_test_d), 1)
...
def train(net, epoch, train_loader, optimizer, scheduler, criterion, lmbda=1e-4):
    ...
    for (samples, labels) in tqdm(train_loader, ncols=70):
        ...
        labels_d = labels[:, 2].squeeze()
        labels_act, labels_loc, labels_d = Variable(labels_act), Variable(labels_loc), Variable(labels_d)
        # meme step
        labels_loc = labels_loc - 1
        optimizer.zero_grad()
        pred_act, pred_loc, domain = net(samples)
        loss_act = criterion(pred_act, labels_act)
        loss_loc = criterion(pred_loc, labels_loc)
        loss_d = criterion(domain, labels_d)
        #############################################################################
        loss = loss_act + loss_loc - lmbda*loss_d
...
def evaluate(net, test_loader):
    ...
    for samples, labels in tqdm(test_loader, ncols=70):
        ...
        with torch.no_grad():
            ...
            labelsV_domain = Variable(labels_d)
            predict_label_act, predict_label_loc, domain = net(samplesV)
            ...
            prediction = domain.data.max(1)[1]
            correct_test_domain += prediction.eq(labelsV_domain.data.long()).sum()
    ...
    acc_dis = float(correct_test_domain) / num_instances
    print("Activity accuracy: {:.2f}%".format(100 * acc_act))
    print("Location accuracy: {:.2f}%\n".format(100 * acc_loc))
    print("Discriminator accuracy: {:.2f}%\n".format(100 * acc_dis))
