...
class DiscriminatorNet(ResNet):
    def __init__(self, block, layers,  inchannel=52, activity_num=6, location_num=16):
        super(DiscriminatorNet, self).__init__(block, layers, inchannel, activity_num, location_num)
    ...
    def forward(self, x):
        ...
        y_hat = torch.cat([act1, loc1], dim=1)
        dis = torch.cat([dis, y_hat], dim=1)
        dis1 = self.disfc(dis)

        return act1, loc1, dis1
